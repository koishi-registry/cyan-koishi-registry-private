...
export async function notFound(name: string) {
  ...
  if (resp.status === 200)
    console.log("%c E %c %s%c is not found", "background-color: red", "color: darkwhite; font-weight: bold", name, "color: red")
  else
    console.log("%c E %c %s%c is not found (!)", "background-color: red", "color: darkwhite; font-weight: bold", name, "color: red", resp.status)
}
...
export const [versions1, versions2] = [obj1, obj2].map(
  (x) =>
    x.map((item) =>
      item.ignored ? null : [item.shortname, [item.package.name, item.package.version]]
    )
      .filter(Boolean),
)
