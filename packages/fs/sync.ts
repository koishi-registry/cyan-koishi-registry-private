...
export function walk(root_: string | URL, options?: Partial<WalkOptions>): Iterable<WalkEntry> {
  let {
  ...
  return readdirSync(root).map(x => [x, lstatSync(join(root, x))] as const).filter(([name, stat]) => {
