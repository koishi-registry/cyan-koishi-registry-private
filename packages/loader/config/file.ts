  ...
  async read(): Promise<EntryOptions[]> {
    switch (this.type) {
      case 'application/yaml': // we assume the type matches
        return yaml.load(await Bun.file(this.name).text(), { schema }) as EntryOptions[]
...
export namespace LoaderFile {
  ...
  for (const extname of Object.keys(Reflect.get(Module, '_extensions') as Record<string, Function>)) {
