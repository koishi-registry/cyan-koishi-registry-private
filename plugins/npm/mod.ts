...
export class NpmSynchronizer extends Service {
  ...
  constructor(
    ctx: Context,
    public options: NpmSynchronizer.Config,
  ) {
    super(ctx, 'npm');
  ...
  async handle(
    ...
    while (this.ctx.scope.active) {
      ...
      if (filtered.length > 0) {
        this.ctx.logger.info(`fetched %C`, filtered.map((x) => x.id).join(' '));
  ...
  private async getMaxSeq(_times = 0): Promise<number> {
    ...
    const response = await this.ctx
      .http<ReplicateInfo>(`${this.options.endpoint}/`, {
  ...
  private async _fastFetch(target_seq: number) {
    ...
    const block_queue = Array.from({ length: blocks_count }, (_, id) => ({
      id,
      begin: this.seq + this.options.block_size * id,
      end: this.seq + this.options.block_size * (id + 1) - 1,
    }));
    const blockMap: Map<number, BlockTask> = new Map(
      // all blocks, for status tracking
      block_queue.map((x) => [x.id, Object.assign(x, { done: false })]),
    );

    const setComplete = (id: number) => {
      // set the status block with this `id` as completed
    ...
    const update = (id: number, seq: number) => {
      // update the current seq of the block with this `id`
  ...
  private _startTask() {
    this.task = this.fetch().catch((e) => {
      // catches error, log the error, finally cancel our scope (dispose the plugin)
