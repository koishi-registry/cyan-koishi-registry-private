...
export type SubObj<T> = T & {
  [K in keyof T as K extends string
    ? keyof T[K] extends string
      ? `${K}.${keyof T[K]}`
      : never
    : never]: T[K][keyof T[K]] extends object
...
> extends Service {
  ...
  async _get(name: string): Promise<toObject | undefined> {
    ...
    const data = await Deno.readFile(join(dir, name), {}).catch((e) =>
  ...
  async has<K extends keyof d<S>>(key_: K): Promise<boolean> {
    ...
    if (!keys.length) {
      const dir = this.getCacheDir(this.keys || [this.ctx.name]);
      const stat = await Deno.lstat(join(dir, name)).catch((e) =>
