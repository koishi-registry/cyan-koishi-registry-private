...
export class StorageBunSqlite extends Storage {
  ...
  constructor(
    ...
    this.database = new Database(
      options?.baseDir || "./data/db.sqlite",
      {
        create: true
      }
    );
    ...
    this.stmts = {
      has: this.database.prepare("SELECT EXISTS(SELECT 1 FROM storage WHERE key = ?)"),
      remove: this.database.prepare("DELETE FROM storage WHERE key = ?"),
      setRaw: this.database.prepare("INSERT OR REPLACE INTO storage (key, value) VALUES (?, ?)"),
  ...
  getRaw(key: string): string | null {
    return <string | null>(<{ value?: string }>this.stmts.getRaw.get(key))?.value
  }
}
