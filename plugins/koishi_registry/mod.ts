...
export class RegistryGenerator extends Service {
  ...
  isSynchronized(): boolean {
    // if all fetches are done, and npm changes is synchronized, then it is real synchronized
  ...
  static isCompatible(
    ...
    try {
      return (
        declaredVersion && rangeIntersects(range, parseRange(declaredVersion))
      );
    } catch {
      return false;
    }
  ...
  private async _generateObject(
      ...
      if (typeof user === 'string') {
        ...
        const matches = user.match(
          createRegExp(
            ...
            maybe(' '),
            exactly('<')
              .and(oneOrMore(char).groupedAs('email'))
              .and('>')
              .at.lineEnd(),
          ),
        );
    ...
    const versions = compatibles.filter(
  ...
  public async refreshPartially(names: string[]) {
    // update the package of each provided records
    this.beforeRefresh();
  ...
  public async refreshFast() {
    ...
    await Promise.all(
      this.object_cache
        .entries()
        .filter(([_, object]) => !!object)
        .map(async ([packageName, object]) => {
          if (!object) return;
          const pack = await this.ctx.koishi.meta.get(packageName);
          if (!pack) {
