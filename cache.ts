...
// 喜欢看类型体操, 下面还有w
export type SubDot<T> = T & {
    [K in keyof T as K extends string ? keyof T[K] extends string ? `${K}.${keyof T[K]}` : never : never]: SubDot<T[K][keyof T[K]]>;
};


export type SubObj<T> = T & {
    [K in keyof T as K extends string ? keyof T[K] extends string ? `${K}.${keyof T[K]}` : never : never]:
...
// deno-lint-ignore no-explicit-any
export class CacheService<S extends { [K: string]: any } = Caches, Name = null> extends Service {
    // @doNotUse: not a real value
    ...
    getCacheDir(name?: string | string[]) {
        ...
        const cacheDir = intercept.cacheDir || this.ctx.info.cacheDir
    ...
    extend<K extends SubSeq<SubList<S>>>(name: K): CacheService<GetRecursive<S, K>>;
    ...
    set<K extends keyof this['Dot']>(key: K, value: this['Dot'][K] extends toObject ? this['Dot'][K] : never): Promise<void>;
    set(key: string, value: toObject): Promise<void>;
    async set(key_: string | string[], value: toObject) {
        if (typeof value !== 'object') throw new Deno.errors.InvalidData("`value` is not an object")
    ...
    async has<K extends keyof this['Dot']>(key_: K): Promise<boolean> {
        ...
        if (!keys.length) {
            ...
            const stat = await Deno.lstat(join(dir, name))
                .catch(e => e instanceof Deno.errors.NotFound ? false : Promise.reject(e))
            return !!stat
        }
