import { Context } from './context.ts'
import { bearerAuth } from 'hono/bearer-auth'
import { verify } from 'paseto-ts/v4'

export const inject = ['hono', 'http']

export const KEY = Deno.env.get("PUBLIC_KEY")
    ?? "k4.public.ZHCAZC7yPzIS42O8SG1luDNVc61rhvbMvUXCkrpVFic"

export const TRUSTED_USER = [
    'cyan',
    'itzdrli'
]

export function apply(ctx: Context) {
    ctx.hono.use("/api/admin/*", bearerAuth({
        async verifyToken(token, _) {
            try {
                const { payload } = verify<{ user: string, version: number }>(KEY, token)
                if (payload.version !== 1) return false
                if (!TRUSTED_USER.includes(payload.user)) {
                    const { exist } = await ctx.http.get<{ exist: boolean }>("/api/auth/user/" + encodeURIComponent(payload.user))
                    return exist
                }
                return true
            } catch {
                return false
            }
        },
    }))
    ctx.hono.post("/api/admin/trigger_full_refresh", async (c) => {
        ctx.inject(['koishi', 'koishi.npm'], async (ctx) => {
            await ctx.koishi.npm.refreshNpm()
        })
    })
    ctx.hono.post("/api/admin/refresh/:name", async (c) => {
        const refresh_meta = !!c.req.query('include_meta')
        const name = c.req.param('name')
        ctx.inject(['koishi', 'koishi.generator', 'koishi.meta'], async (ctx) => {
            ctx.koishi.generator.fetchObject(name, true, refresh_meta)
        })
    })
}
