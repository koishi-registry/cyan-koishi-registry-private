...
import { isEqualBlock, isOnlyTemplateChanged } from './hot'
...
export async function transformMain(
  ...
  const hasTemplateImport =
    descriptor.template && !compiler.isUseInlineTemplate(descriptor, options)
  ...
  if (development) {
    ...
    attachedProps.push([
      '__file',
      JSON.stringify(development ? filename : path.basename(filename)),
    ])
  }
  ...
  if (
    ...
    if (prevDescriptor && isOnlyTemplateChanged(compiler, prevDescriptor, descriptor)) {
...
async function genScriptCode(
  ...
  let scriptCode = `const ${compiler.scriptIdentifier} = { ${vaporFlag} }`
  let map: RawSourceMap | undefined

  const script = compiler.resolveScript(descriptor, options, ssr, customElement)
  if (script) {
    // If the script is js/ts and has no external src, it can be directly placed
    // in the main module.
    if (compiler.canInlineMain(descriptor, options)) {
