...
export function compile(
  ...
  compiler.resolveScript(descriptor, options, ssr, customElement)
  const result = options.compiler.compileTemplate({
    ...resolveTemplateCompilerOptions(compiler, descriptor, options, ssr)!,
    source: code,
  })
...
export function resolveTemplateCompilerOptions(
  ...
  const resolvedScript = compiler.getResolvedScript(descriptor, ssr)
  const hasScoped = descriptor.styles.some((s) => s.scoped)
  const { id, filename, cssVars } = descriptor

  let transformAssetUrls = options.template?.transformAssetUrls
  // compiler-sfc should export `AssetURLOptions`
  let assetUrlOptions: any //: AssetURLOptions | undefined
  if (transformAssetUrls === false) {
    ...
    if (filename.startsWith(options.root)) {
      const devBase = options.core.baseURL.pathname
      assetUrlOptions = {
        base:
          (options.core.baseURL?.origin ?? '') +
          devBase +
