...
import type { ScriptCompiler } from './script'
...
export async function handleHotUpdate(
  ...
  compiler.resolveScript(descriptor, options, false, customElement)
  const scriptChanged = hasScriptChanged(compiler, prevDescriptor, descriptor)
  if (scriptChanged) {
  ...
  if (!isEqualBlock(descriptor.template, prevDescriptor.template)) {
    ...
    if (!scriptChanged) {
      compiler.setResolvedScript(
        descriptor,
        compiler.getResolvedScript(prevDescriptor, false)!,
        false,
      )
    }
...
export function isOnlyTemplateChanged(
  ...
  return (
    !hasScriptChanged(compiler, prev, next) &&
...
function hasScriptChanged(compiler: ScriptCompiler, prev: SFCDescriptor, next: SFCDescriptor): boolean {
  // check for scriptAst/scriptSetupAst changes
  // note that the next ast is not available yet, so we need to trigger parsing
  const prevScript = compiler.getResolvedScript(prev, false)
  const nextScript = compiler.getResolvedScript(next, false)
  ...
  const prevResolvedScript = compiler.getResolvedScript(prev, false)
...
export function handleTypeDepChange(
  ...
  for (const file of affectedComponents) {
    compiler.invalidateScript(file)
