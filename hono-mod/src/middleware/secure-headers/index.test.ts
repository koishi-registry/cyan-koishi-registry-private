...
describe('Secure Headers Middleware', () => {
  ...
  it('all headers enabled', async () => {
    ...
    expect(res.headers.get('Origin-Agent-Cluster')).toEqual('?1')
    expect(res.headers.get('Cross-Origin-Embedder-Policy')).toEqual(
      'require-corp',
    )
  ...
  it('specific headers disabled', async () => {
    ...
    app.use(
      '*',
      secureHeaders({ xFrameOptions: false, xXssProtection: false }),
    )
    app.get('/test', async (ctx) => {
      return ctx.text('test')
    })
  ...
  describe.each([
    ...
    it('CSP with reportTo', async () => {
      ...
      expect(res1.headers.get(cspHeaderName)).toEqual(
      ...
      expect(res2.headers.get(cspHeaderName)).toEqual(
        "default-src 'self'; report-to endpoint-1",
      )
      ...
      expect(res3.headers.get(cspHeaderName)).toEqual(
      ...
      expect(res4.headers.get(cspHeaderName)).toEqual(
    ...
    it('CSP nonce for script-src', async () => {
      ...
      const nonce =
        csp?.match(/script-src 'self' 'nonce-([a-zA-Z0-9+/]+=*)'/)?.[1] || ''
    ...
    it('CSP nonce by app own function', async () => {
      ...
      app.all('*', async (c) => {
        return c.text(
          `script: ${c.get('test-scriptSrc-nonce')}, style: ${
            c.get('test-styleSrc-nonce')
          }`,
        )
      })
