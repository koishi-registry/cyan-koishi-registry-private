    ...
    $url: (
      arg?: S[keyof S] extends { input: infer R }
        ? R extends { param: infer P }
          ? R extends { query: infer Q } ? { param: P; query: Q }
          : { param: P }
        : R extends { query: infer Q } ? { query: Q }
        : {}
        : {},
    ) => URL
  }
...
> extends globalThis.Response {
  ...
  ok: U extends SuccessStatusCode ? true
    : U extends Exclude<StatusCode, SuccessStatusCode> ? false
    : boolean
  status: U
  statusText: string
  headers: Headers
  ...
  json(): F extends 'text' ? Promise<never>
    : F extends 'json' ? Promise<BlankRecordToNever<T>>
    : Promise<unknown>
  text(): F extends 'text' ? (T extends string ? Promise<T> : Promise<never>)
    : Promise<string>
  blob(): Promise<Blob>
...
type InferEndpointType<T> = T extends (
  ...
  ? U extends ClientResponse<infer O, infer S, infer F>
    ? { input: NonNullable<R>; output: O; outputFormat: F; status: S } extends
      Endpoint
...
type InferResponseTypeFromEndpoint<T extends Endpoint, U extends StatusCode> =
  T extends {
    output: infer O
    status: infer S
  } ? S extends U ? O
...
export type InferRequestType<T> = T extends (
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  options: any | undefined,
) => Promise<ClientResponse<unknown>> ? NonNullable<R>
  : never
...
> = Path extends `/${infer P}` ? PathToChain<P, E, Path>
