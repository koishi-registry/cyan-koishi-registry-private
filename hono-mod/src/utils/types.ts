...
export type JSONParsed<T> = T extends { toJSON(): infer J }
  ...
  : T extends object ? {
      [
        K in keyof OmitSymbolKeys<T> as IsInvalid<T[K]> extends true ? never
          : K
      ]: boolean extends IsInvalid<T[K]> ? JSONParsed<T[K]> | undefined
        : JSONParsed<T[K]>
    }
  : never
...
export type HasRequiredKeys<BaseType extends object> =
  RequiredKeysOf<BaseType> extends never ? false
    : true
